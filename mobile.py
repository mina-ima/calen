...
st.set_page_config(
    page_title="スケジュール帳",
    layout="wide",
    initial_sidebar_state="expanded"  # ← 追加：PC／モバイルともにサイドバーを常に開いた状態に
)
    ...
    with tab_login:
        ...
        if st.button("ログイン"):
            ...
            if account:
                ...
                st.rerun()  # ← experimental_rerun → rerun に変更
            else:
...
with st.expander("表示アカウントの追加と削除"):
...
with st.sidebar:
    ...
    for acc in st.session_state.visible_accounts:
        ...
        st.markdown(
            ...
            f"<div style='width:12px;height:12px;background:{color};margin-right:5px;'></div>{acc}</div>",
    ...
    if st.session_state.eventClick:
        ...
        if not selected.empty:
            ...
            with st.form(f"edit_{clicked_id}"):
                ...
                # 既存ファイル表示
                if isinstance(row["ファイルパス"], str) and row["ファイルパス"].strip():
                    try:
                        ...
                        st.warning(f"ファイル表示エラー: {e}")

                new_file = st.file_uploader("添付ファイルを変更", key=f"edit_file_{clicked_id}")
                submitted_update = st.form_submit_button("更新", disabled=not editable)
                submitted_delete = st.form_submit_button("削除", disabled=not editable)
